...
main() {
  ...
  prev_version="$1"; release_type="$2"; prerelease_type="$3";

  if [[ "$prev_version" == "" ]]; then
    echo "could not read previous version"; exit 1
  fi

  possible_release_types="NONE MAJOR MINOR PATCH"

  if [[ ! ${possible_release_types[*]} =~ ${release_type} ]]; then
    echo "valid argument: [ ${possible_release_types[*]} ]"; exit 1
  fi

  possible_prerelease_types="REMOVE ALPHA BETA RC"

  if [[ ! ${possible_prerelease_types[*]} =~ ${prerelease_type} ]]; then
    echo "valid argument: [ ${possible_prerelease_types[*]} ]"; exit 1
  fi
  ...
  echo ::set-output name=updated-version::"$next_version"
}
